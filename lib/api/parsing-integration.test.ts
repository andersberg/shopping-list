...
describe("Parsing Integration Tests", () => {
  ...
  describe.each(TEST_CASES)("$description", ({ input, expected }) => {
    it(`should parse: "${input}"`, async () => {
      ...
      console.log(`Parsed: item="${result.item}", category="${result.category}", quantity=${result.quantity}, unit="${result.quantity_unit}"`);
  ...
  it("should handle all test lines without errors", async () => {
    ...
    const parseErrors = results.filter((r) => r.status === "parse_error").length;

    console.log(
      `Parsing results: ${successful} successful, ${needsReview} need review, ${parseErrors} errors`,
    );
  ...
  it("should maintain consistency across similar patterns", async () => {
    ...
    const weightTests = ["1 kg potatis", "potatis 1 kilo", "mjölk 2 liter"];

    const results = await Promise.all(
      weightTests.map(async (line) => {
        const response = await fetch(API_URL, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ input: line }),
