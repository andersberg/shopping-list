...
export type GroceryAiExtraction = z.infer<typeof GroceryAiExtractionSchema>;

// Re-export the unified schema
export const GroceryItemSchema = grocery_item_full_schema;
...
function map_item_to_canonical(item: string | null): { canonical: string | null; category: string | null } {
...
function parse_offer(offer: string | null): {
  ...
  const slashMatch = offer.match(/(\d+)\s*\/\s*(\d+(?:\.\d+)?)\s*kr?/i);
  if (slashMatch) {
    const quantity = parseInt(slashMatch[1]);
  ...
  const forMatch = offer.match(/(\d+)\s*för\s*(\d+(?:\.\d+)?)\s*kr?/i);
  if (forMatch) {
...
function extract_organic(modifiers: string[]): boolean {
  return modifiers.some(mod => 
    ["eko", "ekologisk", "ekologiska", "organic", "krav"].includes(mod.toLowerCase())
  );
}

export function map_tokens_to_grocery_item(tokens: GroceryAiExtraction): GroceryItem {
  try {
    ...
    // Parse quantities
    const quantity = tokens.raw_qty ? parseInt(tokens.raw_qty) || 0 : 0;
    ...
    if (brand && BRAND_NAMES.some(known_brand => normalize_string(known_brand) === brand)) {
      // Use the original casing from BRAND_NAMES
      brand = BRAND_NAMES.find(known_brand => normalize_string(known_brand) === brand) || null;
    } else if (brand) {
