...
export function print_console_report(report: AuditReport, verbose: boolean = false): void {
  // Header
  ...
  if (report.status === 'server_unavailable') {
    console.log(pc.red(`${SYMBOLS.fail} Server not available at ${report.api_url}`));
...
function print_test_result(
  ...
  console.log(`${color(`[${index}/${total}] ${symbol} ${result.description}`)}`);
  console.log(pc.dim(`  Input: "${(result.expected as any)?.raw_text || (result.actual as any)?.raw_text || 'unknown'}"`));
  ...
  if (result.mismatches && result.mismatches.length > 0) {
    for (const [idx, mismatch] of result.mismatches.entries()) {
      ...
      console.log(`    ${pc.dim(prefix)} ${pc.cyan(`Field: ${mismatch.field}`)}`);
      console.log(`    ${pc.dim('│')}  ${pc.green('Expected:')} ${format_value(mismatch.expected)}`);
      console.log(`    ${pc.dim('│')}  ${pc.red('Actual:  ')} ${format_value(mismatch.actual)}`);
...
function print_final_summary(report: AuditReport): void {
  ...
  console.log(`  ${pc.green(SYMBOLS.pass)} Pass:           ${pass}  (${percentage(pass, report.total_tests)})`);
  console.log(`  ${pc.red(SYMBOLS.fail)} Mismatch:       ${mismatch}  (${percentage(mismatch, report.total_tests)})`);
  console.log(`  ${pc.red(SYMBOLS.http_error)} HTTP Error:     ${http_error}  (${percentage(http_error, report.total_tests)})`);
  console.log();

  const issues_count = mismatch + http_error;
  if (issues_count > 0) {
    console.log(`Status: ${pc.red('NEEDS ATTENTION')} (${issues_count} issue${issues_count > 1 ? 's' : ''} found)`);
  } else {
...
function get_status_color(status: AuditResult['status']): (text: string) => string {
  switch (status) {
    case 'success':
      return pc.green;
    case 'mismatch':
...
function get_status_text(status: AuditResult['status'], result: AuditResult): string {
  switch (status) {
    case 'success':
