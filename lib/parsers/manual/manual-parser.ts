...
export function post_process(draft: ParsedDraft): ParsedResult {
	...
	const { canonical: canonical_item, category } = canonicalize_item(draft.item);
	const { store_normalized, store_raw } = extract_store_from_text(
		draft.item || "",
	);
	const { quantity: final_quantity, quantity_unit: final_quantity_unit } =
		apply_default_quantity(
			canonical_item,
			draft.quantity ?? 0,
			draft.quantity_unit,
	...
	// Use category from shared logic, fallback to null for type compatibility
	const final_category = category || null;
	const { quantity, quantity_unit } = apply_default_quantity(
		canonical_item,
		draft.quantity ?? 0,
		draft.quantity_unit,
		draft.size_value ?? 0,
	...
	const offer_data = parse_offer(
		draft.offer_currency
			? `${draft.offer_quantity}/${draft.offer_total_price_value}kr`
			: null,
	);
	...
	// Determine status
	const status = determine_status(canonical_item, category, false);

	return {
		item: canonical_item,
		category: final_category as any, // Cast for type compatibility
		quantity: final_quantity,
		quantity_unit: final_quantity_unit as any,
		size_value,
		size_unit: draft.size_unit as any,
		brand: normalize_brand(draft.brand),
		organic: draft.organic || extract_organic([]), // TODO: extract from modifiers
		comment: draft.comment,
		unit_normalized: null, // TODO: normalize using unit lexicon if needed
		total_quantity_value,
		total_quantity_unit: total_quantity_unit as any,
		store_normalized: store_normalized as any,
		store_raw: store_raw,
		offer_quantity: offer_data.offer_quantity,
		offer_total_price_value: offer_data.offer_total_price_value,
		offer_currency: offer_data.offer_currency as any,
		offer_unit_price_value: offer_data.offer_unit_price_value,
		status,
		error: status === "parse_error" ? "No item candidate found" : null,
		// Database fields (legacy compatibility)
		name: canonical_item,
		discount_price: null,
		unit: final_quantity_unit,
		source: "manual",
	};
}
...
export class ManualParser implements GroceryParser {
	readonly source = "manual" as const;

	parse(input: string): GroceryItem {
		// 1. Pre-normalize
